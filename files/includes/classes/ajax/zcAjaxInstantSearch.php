...
class zcAjaxInstantSearch extends base
{
     ...
     * @var string
     */
    protected const string TYPESENSE_INSTANT_SEARCH_CLASS_NAME = TypesenseInstantSearch::class;
    ...
    public function instantSearch(): string
    {
        ...
        if ($_POST['scope'] === 'dropdown') {
            ...
            // If a custom sort is applied, set the SQL limit to the maximum value.
            // We need to fetch all the products from the database in order to properly sort them,
            // otherwise at every "ajax page" loade the displayed results would change.
            ...
            if (!empty($result['products_id'])) {
                ...
                $dropdownResult['count'] = INSTANT_SEARCH_DROPDOWN_DISPLAY_MANUFACTURERS_COUNT === 'true'
                    ? !empty($result['manufacturers_count'])
                        ...
                        : isearch_count_products_for_manufacturer((int)$id)
                    : '';
            } else {
                continue;
            }
    ...
    {
        // $tplSetting used in ZC3 product_listing
        global $zco_notifier, $current_page_base, $cPath, $request_type, $template, $db, $tplSetting;
